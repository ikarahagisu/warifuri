...
with st.sidebar:
    ...
    * **割振後max患者数**: 上限人数です。
    * **大変さのmaxスコア**: 受け入れ可能な重症度制限です。
    
    ### 2. 患者リストの準備
    右側の表に、割り振りたい患者を入力します。
    * **CSV読込**: 一括取り込みが可能です。
    * **直接編集**: 名前やスコアを直接書き換え・コピペできます。
    
    ### 3. 割り振りの実行
    中央のボタンを押してください。新しく増える「大変さ」の合計が均等になるよう自動計算します。
    
    ### 4. 結果の確認
    一番下の表に入力順で担当医が表示されます。CSV保存も可能です。
    """)
    st.divider()
    st.caption("ver 1.5 - レイアウト復元版")
...
with col1:
    ...
    st.info(f"💡 ヒント: 全患者({total_current}名)＋新規({total_new}名)を{doc_count}名で割ると、1人あたり約 {avg_target:.1f} 名です。")
