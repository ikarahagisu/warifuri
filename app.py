...
with st.sidebar:
    ...
    ### 1. 医師の状態を設定
    左側の表で、各医師の**「現在の患者数」**を入力します。
    * **割り振り後max患者数**: その医師が持てる合計人数の上限です。
    ...
    * **ランダム性**: ボタンを押すたびに組み合わせが変わります。納得がいくまで数回押して調整可能です。
    
    ### 4. 結果の確認と保存
    * **個別患者の割り振り結果**: 入力した順番のまま担当医が表示されるので、電子カルテへの転記（突合）に便利です。
    * **CSV保存**: 結果をExcelで開ける形式でダウンロードできます。
    """)
    st.divider()
    st.caption("ver 1.0 - 現場の負担軽減のために")
...
if "doctors_df" not in st.session_state:
    st.session_state.doctors_df = pd.DataFrame([
        {"名前": "医師A", "現在の患者数": 8, "割り振り後max患者数": 15, "大変さのmaxスコア": 5},
        {"名前": "医師B", "現在の患者数": 3, "割り振り後max患者数": 15, "大変さのmaxスコア": 5},
        {"名前": "医師C", "現在の患者数": 2, "割り振り後max患者数": 15, "大変さのmaxスコア": 5},
...
if st.button("このデータで患者を割り振る", type="primary"):
    ...
    for p in sorted_patients:
        eligible_docs = [
            d for d in doctors 
            if d["現在の患者数"] < d["割り振り後max患者数"] and p["大変さスコア"] <= d["大変さのmaxスコア"]
        ]
    ...
    if len(unallocated) == 0:
        ...
        st.warning(f"⚠️ 全 {valid_patients_count} 名のうち {allocated_count} 名が完了、{len(unallocated)} 名が未割り当てです。")
    ...
    with res_col1:
        ...
        display_df = df_docs_final[["名前", "割り振り後総患者数", "追加患者数", "新規追加スコア合計", "割り振り後max患者数"]]
    ...
    with res_col2:
        ...
        if unallocated:
            st.error(f"⚠️ 未割り当て: {', '.join([p['名前'] for p in unallocated])}")
    ...
    with col_btn:
        st.download_button(label="📥 この表をCSVでダウンロード", data=csv, file_name='allocation_result.csv', mime='text/csv', use_container_width=True)
